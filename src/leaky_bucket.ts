...
export interface LeakyBucketContext {
...
export class LeakyBucketEngine {
  private registry = new Map<string, any>();
  private processed = 0;
  private errors = 0;

  constructor(private readonly config: LeakyBucketConfig = { timeoutMs: 5000, maxRetries: 3, enabled: true }) {}
  ...
  public async process(ctx: LeakyBucketContext): Promise<{ digest: string; durationMs: number }> {
