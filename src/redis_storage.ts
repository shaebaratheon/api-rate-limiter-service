...
export interface RedisStorageContext {
...
export class RedisStorageEngine {
  ...
  constructor(private readonly config: RedisStorageConfig = { timeoutMs: 5000, maxRetries: 3, enabled: true }) {}
  ...
  public async process(ctx: RedisStorageContext): Promise<{ digest: string; durationMs: number }> {
