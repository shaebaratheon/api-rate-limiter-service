...
export interface TieredTiersConfig {
  timeoutMs: number;
  maxRetries: number;
  enabled: boolean;
}

export interface TieredTiersContext {
...
export class TieredTiersEngine {
  private registry = new Map<string, any>();
  private processed = 0;
  private errors = 0;

  constructor(private readonly config: TieredTiersConfig = { timeoutMs: 5000, maxRetries: 3, enabled: true }) {}
  ...
  public async process(ctx: TieredTiersContext): Promise<{ digest: string; durationMs: number }> {
